...
Für die binaurale Wiedergabe wird eine HRTF-Datenbank verwendet. HRTF steht für \textit{Head-Related Transfer Function} und beschreibt, wie ein eintreffender Schall durch Kopf, Ohrmuscheln und Oberkörper abhängig von seiner Richtung verändert wird. Diese Filtereffekte sind entscheidend dafür, dass Menschen Schallquellen räumlich lokalisieren können.

In der Implementierung wird eine SOFA-Datei aus der CIPIC-Datenbank verwendet, beispielsweise \texttt{subject\_003.sofa}. SOFA ist ein standardisiertes Format für räumliche Audiodaten. Die Klasse \texttt{HRTFDatabase} lädt daraus die Messpositionen und die dazugehörigen HRIRs. HRIR steht für \textit{Head-Related Impulse Response} und bezeichnet die Impulsantworten für linkes und rechtes Ohr zu einer bestimmten Einfallsrichtung.
...
Diese Version verwendet dafür eine einfache Nearest-Neighbor-Auswahl. Das bedeutet, dass nicht zwischen Messpunkten interpoliert wird, sondern immer die nächstgelegene vorhandene HRIR genutzt wird. Dadurch bleibt die Implementierung robust und leicht nachvollziehbar.
...
Dadurch wird jede Reflexion nicht nur zeitlich verzögert und abgeschwächt, sondern zusätzlich durch einen richtungsabhängigen Ohrfilter geprägt. Aus allen Ray-Hits entsteht eine zweikanalige binaurale Impulsantwort. Diese wird anschließend mit dem trockenen Eingangssignal gefaltet:
...
Das Ergebnis ist eine Stereo-WAV-Datei. Über Kopfhörer kann diese Datei als räumliches Echo bzw. räumlicher Nachhall wahrgenommen werden.
...
Die Erweiterung macht aus der klassischen Impulsantwort eine gerichtete binaurale Impulsantwort. Damit wird das simulierte Echo nicht nur zeitlich und energetisch, sondern auch räumlich wiedergegeben.