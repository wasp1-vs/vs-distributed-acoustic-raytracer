...
Die eigentliche Signalverarbeitung erwies sich als der algorithmisch unkritischste Teil der Pipeline; die zentrale Herausforderung war organisatorischer Natur.
So existierten zwischenzeitlich zwei unabhängig entstandene Verfahren für die räumliche Wiedergabe: ein einfaches Azimut-Panning in \texttt{convolution.py} sowie eine vollständige HRTF-Faltung über SOFA-Datensätze in \texttt{binaural\_convolver.py}.
Die spätere Konsolidierung verdeutlichte, dass paralleles Arbeiten an derselben Schnittstelle ohne frühe Absprache zu Doppelentwicklungen führt.
Eine weitere Lehre betraf die Kalibrierung der Simulationsparameter.
Ein überdimensionierter virtueller Mikrofonradius von $3\,\text{m}$ in einem Raum generierte riesige JSON-Datenmengen ($>1\,\text{GB}$).
Ein physikalisch plausibler Radius ($0{,}5\,\text{m}$) verringerte die I/O-Last drastisch.
Eine numerisch \quotedblbase funktionierende\textquotedblleft{} Simulation ist ohne realistische Standardparameter oftmals unbrauchbar.
