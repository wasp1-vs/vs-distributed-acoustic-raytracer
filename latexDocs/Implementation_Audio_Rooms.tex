...
Das System folgt einer 4-stufigen Pipeline:

\begin{enumerate}
    \item[\textbf{[1/4]}] Rust Physics Engine: Simuliert Schallstrahlen, erzeugt Impulsantwort (IR)
    \item[\textbf{[2/4]}] Audio-Einlesung: L\"adt trockene WAV-Datei
    \item[\textbf{[3/4]}] DSP-Convolution: Wendet IR auf Audio an
    \item[\textbf{[4/4]}] Audio-Export: Normalisiert und speichert Ergebnis
\end{enumerate}

Die Raumdaten werden parallel in den Tracer geladen, um realistische 3D-Geometrien zu unterst\"utzen.
...
\section{Export ins JSON-Format}

\begin{lstlisting}
import json
triangles = load_obj_to_triangles("room.obj")
data = {
    "triangles": triangles,
    "triangle_count": len(triangles)
}
with open("room_geometry.json", "w") as f:
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|c|}
        ...
        \hline
        \texttt{obj\_parser.py} & pathlib & Built-in \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{4.5cm}|p{2cm}|p{5cm}|}
        ...
        \hline
        JSON-Export & \checkmark & G\"ultige JSON, Geometrie-Metadaten \\
        \hline
    \end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Pipeline-Integration}: Nahtlose Verkettung aller Komponenten
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Material-Attribute}: Pro-Dreieck Material-IDs aus OBJ
